  ...
  changeWorkspaceRoot() {
    setOrChangeWorkspace().then((workspceName) => {
      ...
      window.showInformationMessage(
        `Success! '${workspceName}' workspace is now root of Live Server`
      );
...
async function getQuickPromptPick(items: QuickPickItem[]) {
  if (items.length === 0) {
    return;
  }
  if (items.length === 1) {
  ...
  const configPicked = await window.showQuickPick(items);
  if (configPicked) {
    return configPicked.description;
  }
  return;
}
