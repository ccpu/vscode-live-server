...
function staticServer(root, onTagMissedCallback, options) {
  var isFile = false;
  try {
      ...
      transform: function (stream, res, opts) {
        ...
        stream.pipe(
          concatStream(function (d) {
            let content = d.toString('utf8');

            if (injectTag) {
              const optionContent = GET_INJECTED_CODE();
              content = content.replace(
                new RegExp(injectTag, 'i'),
                optionContent + injectTag
              );
            }

            if (options.transform) {
              for (let i = 0; i < options.transform.length; i++) {
                const t = options.transform[i];
                const from = t.replace;
                const to = t.to.join('\n');
                if (content.match(from)) {
                  content = content.replace(new RegExp(from), to);
                }
              }
            }

            var _d = Buffer.from(content);
...
LiveServer.start = function (options, callback) {
  ...
  var staticServerHandler = staticServer(root, onTagMissedCallback, options);
